...
public class InvoiceController {
    ...
    public ResponseEntity<List<Invoice>> getInvoicesByUserId(HttpServletRequest request) {
        final Long JWT_USER_ID = Utilities.returnUserIdFromJwt(request, jwtDecoder);
        return ResponseEntity.ok(invoiceService.getInvoicesByUserId(JWT_USER_ID));
    }

    @GetMapping("/merchant/invoices")
    public ResponseEntity<List<Invoice>> getInvoicesByMerchantId(HttpServletRequest request) {
        final Long JWT_USER_ID = Utilities.returnUserIdFromJwt(request, jwtDecoder);
        return ResponseEntity.ok(invoiceService.getInvoicesByMerchantId(JWT_USER_ID));
    }

    @GetMapping("/merchant/{merchantId}/invoices")
    public ResponseEntity<List<Invoice>> getInvoicesByMerchantIdAndUserId(@PathVariable Long merchantId, @RequestParam(required = false) String exclude, HttpServletRequest request) {
        final Long JWT_USER_ID = Utilities.returnUserIdFromJwt(request, jwtDecoder); // TODO fix this as the same as above
        return ResponseEntity.ok(invoiceService.getInvoicesByMerchantIdAndUserId(merchantId, JWT_USER_ID, exclude));
    }
